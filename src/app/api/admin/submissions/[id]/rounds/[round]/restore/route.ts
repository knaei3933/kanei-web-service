...
/* ------------------------------------------------------------------ */
/*  POST /api/admin/submissions/[id]/rounds/[round]/restore — 復元     */
/* ------------------------------------------------------------------ */
/*  役割:                                                              */
/*    過去ラウンド N の内容を新規ラウンドとして復元する。                */
/*    snapshots/round-N.json から componentSource を取得し、              */
/*    復元用 revision-handoff.json を生成して lineage に記録する。        */
...
export async function POST(
  ...
  try {
    ...
    await writeArtifact(
      ...
      JSON.stringify(restoreHandoff, null, 2)
    );

    // lineage に復元ラウンドを追加（hasComponentSource=true は snapshot にあるため）
    ...
    // demo_generating 中なら pending handoff を新しい restore に置き換える意味で直接巻き戻す
    ...
    return Response.json({
      ...
      message: `ラウンド ${round} からラウンド ${nextRound} として復元しました`,
    });
  } catch (error) {
    return Response.json(
      {
